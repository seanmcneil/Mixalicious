...
public final class Mixalicious: ObservableObject {
    ...
    @Published var completionPercent = 0.0
    ...
    private let timerPublisher = Timer.publish(every: 0.1,
                                               on: .main,
                                               in: .default)
    /// Suppors cancelling the sink that handles the publisher.
    ...
    public func insert(audio: AVAsset,
        ...
        guard insertionTime >= .zero,
            insertionTime <= target.timeRange.end else {
    ...
    /// Extracts the audio track from the provided video into a new file
    ///
    /// - Note: This does not affect the video file
    /// - Note: The audio track will be returned as an m4a file
    ///
    /// - Parameters:
    ...
    /// - Note: If the videos array contains any non-video assets, it will fail
    ///
    /// - Parameters:
    ...
    public func combineVideos(videos: [AVAsset]) -> AnyPublisher<URL, MixaliciousError> {
        guard !videos.isEmpty,
            videos.filter({ !$0.hasVideoTrack }).isEmpty else {
            return Fail(error: .videoTrackNotFound)
                .eraseToAnyPublisher()
        }
